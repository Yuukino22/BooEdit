...
public:
    ...
    //Insert a character where the cursor is currently positioned.
    void insertCharacter(char c);

    //Undo insertCharacter().
    void deleteCharacter();

    //Create newline under the current line.
    void newLine();
    ...
    //Undo cursorUp().
    void undoUp(int col, int line);

    //Move cursor down one cell.
    void cursorDown();

    //Undo cursorDown().
    void undoDown(int col, int line);

    //Move cursor to the beginning of the current line.
    void cursorHome();

    //Undo cursorHome().
    void undoHome(int col);

    //Move cursor to the end of the current line.
    void cursorEnd();

    //Undo cursorEnd().
    void undoEnd(int col);

    //Delete to the left of the current cursor position.
    void backSpace();

    //Undo backSpace().
    void undoBackSpace(char c, int col, int line);

    //Delete the entire current line.
    void deleteLine();

    //Undo deleteLine().
    void undoDeleteLine(std::string s, int col, int line, int linecount);

    //Delete every pointer in content.
    void clearContent();
...
private:
    ...
    std::vector<std::string*> content{new std::string("")};
};
